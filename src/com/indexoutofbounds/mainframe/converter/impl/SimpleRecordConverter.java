...
public class SimpleRecordConverter implements IRecordConverter {

	private static Logger log = Logger.getLogger(SimpleRecordConverter.class);
	 ...
	 */
	public SimpleRecordConverter(String filename, FileRecordLayout recordLayout) throws MainFrameConversionException {

		try {

			log.debug("SimpleRecordConverter constructor");
	...
	public void verifyFileIsClosed() throws MainFrameConversionException {
		...
		try {
			...
			/* Ignore because we should be expecting this exception.
			 * Trying to close something that's already closed would throw an exception
	...
	private byte[] getNextRecordsBytes() throws MainFrameConversionException {
		...
		try {
			if (bufferedInputStream.read(bytes, 0, bytesToRead) > 0) {
				...
				/*
				 * TODO What happens if we can't read in the full COBOL record amount?
				 * Valid 'amounts' are -1 (hit EOF) or the full amount...
				 */

				if (bytes.length != bytesToRead) {
					throw new MainFrameConversionException(MainFrameGlobals.SimpleRecordConverter_BytesReadNotFulLRecord);
				}
	...
	/**
	 * Returns a HashMap of converted fields from a MainFrame record.
	 * We only convert those Fields who are marked with a isProcess of true.
	 * 
	 * @param rowbytes
	 * @return HashMap
	 * @throws MainFrameConversionException
	...
	private Field translateField(FileRecordFieldLayout frfl, byte[] fieldBytes) throws MainFrameConversionException {
		...
		// We have to wrap in a try/catch in order to stick the field name in the Exception message
