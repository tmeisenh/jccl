...
public class BaseCobolUtils {
	...
	protected static boolean isNegativeHighNybbleSignBit(byte signbit) throws MainFrameConversionException {

		int sign = digOutHighNybbleValue(signbit);
		
		if(!checkSign(sign)) {
			throw new MainFrameConversionException(MainFrameGlobals.COBOLConversionUtils_SignBit);
		}
	 ...
	 * @throws MainFrameConversionException
	 */
	protected static int digOutSignMultiplierHighNybble(byte signbit) throws MainFrameConversionException {
	...
	protected static byte[] packDataIntoBytes(String val) {
		...
		for (int i = 0; i < val.length(); i += 2) {
			 ...
			 */
            log.debug("PACKING: " + val.substring(i, i + 2));
