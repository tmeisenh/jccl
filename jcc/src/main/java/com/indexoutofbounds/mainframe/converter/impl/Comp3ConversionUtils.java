...
public class Comp3ConversionUtils extends BaseCobolUtils {
	 ...
	 * @throws Sock7Exception
	 */
	public static byte[] convertDoubleToCOMP3(double val, int byteLength, int scale, boolean isSigned)
			throws Sock7Exception {
	...
	private static int[] unpackCOMP3Byte(byte b) throws Sock7Exception {
		...
		if (STRICT_TRANSLATION != 0) {
			...
			if (cbyte[0] > 9 || cbyte[1] > 9)
				throw new Sock7Exception(MainFrameGlobals.COBOLConversionUtils_COMP3_Value + " [" + cbyte[0] + ":"
						+ cbyte[1] + "]");
		}
		return cbyte;
	}
	 ...
	 * @throws Sock7Exception
	 */
	private static String normalizeJavaDataIntoCOMP3ByteString(String value, int byteLength, int scale, char sign)
			throws Sock7Exception {
